...
class ExampleFullArmMovement:
    ...
    def go_to_cartesian_pose(self, x, y, z, theta_x, theta_y, theta_z, blending_radius, pose_name):
        # Going to top left pose
        self.last_action_notif_type = None
        # Get the actual cartesian pose to increment it
        # You can create a subscriber to listen to the base_feedback
        # Here we only need the latest message in the topic though
        ...
        req = ExecuteActionRequest()
        trajectory = WaypointList()
        trajectory.waypoints.append(self.FillCartesianWaypoint(x, y, z, theta_x, theta_y, theta_z, blending_radius))
        req.input.oneof_action_parameters.execute_waypoint_list.append(trajectory)
        # Call the service
        rospy.loginfo(f"Sending the robot to the {pose_name} pose...")
        try:
            self.execute_action(req)
        except rospy.ServiceException:
            rospy.logerr(f"Failed to go to the {pose_name} pose")
            return False
        else:
            return self.wait_for_action_end_or_abort()
    ...
    def main(self):
        ...
        if success and not object_found:
            # Going to top left pose
            success &= self.go_to_cartesian_pose(0.4, -0.25, scan_height, 180, 0, 90, 0, "top left")
            rospy.loginfo(success)
        
        if success and not object_found:
            # Going to middle left pose
            success &= self.go_to_cartesian_pose(0.2, -0.25, scan_height, 180, 0, 90, 0, "middle left")
            rospy.loginfo(success)
        
        if success and not object_found:
            # Going to bottom left pose
            success &= self.go_to_cartesian_pose(0.0, -0.25, scan_height, 180, 0, 90, 0, "bottom left")
            rospy.loginfo(success)

        
        if success and not object_found:
            # Going to bottom middle pose
            success &= self.go_to_cartesian_pose(0.0, -0.375, scan_height, 180, 0, 90, 0, "bottom middle")
            rospy.loginfo(success)

        if success and not object_found:
            # Going to middle middle pose
            success &= self.go_to_cartesian_pose(0.2, -0.375, scan_height, 180, 0, 90, 0, "middle middle")
            rospy.loginfo(success)

        if success and not object_found:
            # Going to top middle pose
            success &= self.go_to_cartesian_pose(0.4, -0.375, scan_height, 180, 0, 90, 0, "top middle")
            rospy.loginfo(success)

        if success and not object_found:
            # Going to top right pose
            success &= self.go_to_cartesian_pose(0.4, -0.5, scan_height, 180, 0, 90, 0, "top right")
            rospy.loginfo(success)

        if success and not object_found:
            # Going to middle right pose
            success &= self.go_to_cartesian_pose(0.2, -0.5, scan_height, 180, 0, 90, 0, "middle right")
            rospy.loginfo(success)

        if success and not object_found:
            # Going to bottom right pose
            success &= self.go_to_cartesian_pose(0.0, -0.5, scan_height, 180, 0, 90, 0, "bottom right")
            rospy.loginfo(success)

        if success and not object_found:
            # Going to top right pose to go home
            success &= self.go_to_cartesian_pose(0.4, -0.5, scan_height, 180, 0, 90, 0, "top right")
            rospy.loginfo(success)

        if success and not object_found:
            # Going to top left pose to go home
            success &= self.go_to_cartesian_pose(0.4, -0.25, scan_height, 180, 0, 90, 0, "top left")
            rospy.loginfo(success)
        ...
        if object_found:
            ...
            if success:
                ...
                feedback = rospy.wait_for_message("/" + self.robot_name + "/base_feedback", BaseCyclic_Feedback)
                success &= self.go_to_cartesian_pose(
                    feedback.base.commanded_tool_pose_x + x_offset,
                    feedback.base.commanded_tool_pose_y + y_offset,
            ...
            z_offset = 0.16
            if success:
                # Currently above object
                feedback = rospy.wait_for_message("/" + self.robot_name + "/base_feedback", BaseCyclic_Feedback)
                success &= self.go_to_cartesian_pose(
                    feedback.base.commanded_tool_pose_x,
                    feedback.base.commanded_tool_pose_y,
                    feedback.base.commanded_tool_pose_z - z_offset,
                    feedback.base.commanded_tool_pose_theta_x,
                    feedback.base.commanded_tool_pose_theta_y,
                    feedback.base.commanded_tool_pose_theta_z,
                    0,
            ...
            if success:
                # Opening the gripper
                if self.is_gripper_present:
                    rospy.loginfo("Letting go of the object")
                    success &= self.example_send_gripper_command(0.0)
                    rospy.loginfo(success)
                else:
                    rospy.logwarn("No gripper is present on the arm.")
        ...
        # Always move the robot to home at the end
        if success:
            # Move to the robot to home to end
            success &= self.example_home_the_robot()
